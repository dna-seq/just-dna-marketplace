...
from just_dna_marketplace.storage.hf import HfStorage


def test_hf_paths_and_resolve_url() -> None:
    s = HfStorage("just-dna-seq/marketplace", token=None)
    ...
    assert s._fs_path(key, "weights.parquet") == (
        "datasets/just-dna-seq/marketplace/data/just-dna-seq/coronary/1.0.0/weights.parquet"
    )
    assert s.file_url(key, "logs/reviewer.log") == (
        "https://huggingface.co/datasets/just-dna-seq/marketplace/resolve/main/"
...
def test_hf_backend_selected_by_config() -> None:
    ...
    from just_dna_marketplace.api.app import _build_storage
